...
\begin{enumerate}
    ...
    \item Does the picture change in the black-box setting, where the attacker has no gradient access? I use HopSkipJump (decision-based) and One Pixel ($L_0$) to test this.
    ...
    \item How do the five attacks compare in terms of attack success rate, perturbation magnitude, and computational cost?
\end{enumerate}
...
\begin{itemize}
    \item Implement FGSM, BIM/PGD, DeepFool, HopSkipJump, and One Pixel from scratch in PyTorch and evaluate them on MNIST and CIFAR-10.
    ...
    \item Compare all five attacks under identical conditions --- same model, same data, same evaluation protocol --- spanning $L_\infty$, $L_2$, and $L_0$ threat models.
\end{itemize}
...
\begin{enumerate}
    \item I provide a controlled comparison of three adversarial attacks on the same model with the same evaluation setup. This addresses the difficulty of comparing results across papers that use different configurations.
