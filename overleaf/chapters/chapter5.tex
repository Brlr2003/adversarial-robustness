...
I trained two ResNet-18 models on CIFAR-10. Same architecture, same optimizer, same number of epochs. The only difference is that one was trained normally and the other with PGD adversarial training. Table~\ref{tab:training_config} has the full settings.
...
Against the standard model, DeepFool fools 91.1\% of correctly classified images (Figure~\ref{fig:attack_deepfool}) with an average $L_2$ perturbation of just 0.248. That is a very small number. Against the adversarially trained model, it needs about $3.6\times$ more perturbation on average, and even then the success rate only reaches 62.0\%. So the decision boundaries really have moved further from the data after adversarial training.
...
Playing with the demo, I noticed something interesting about the confidence scores. When the standard model gets fooled, it is not just wrong, it is wrong with 99\% confidence. The adversarially trained model is more cautious. Even when it gets the answer right on an adversarial input, it usually gives something like 50 to 60\% confidence instead of being overly sure. That is actually a useful property because it means the model ``knows'' when it is dealing with something unusual.
