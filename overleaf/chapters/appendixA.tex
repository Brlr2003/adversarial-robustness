...
This appendix provides the core Python implementations for the adversarial attack algorithms, training procedures, and model architecture discussed in Chapter~\ref{ch:implementation}. The complete codebase is available at:

\noindent\url{https://github.com/Brlr2003/adversarial-robustness}
...
\section{Carlini--Wagner Attack Implementation}

\begin{lstlisting}[language=Python, caption={Carlini--Wagner $L_2$ Attack Implementation}, label={lst:cw}]
import torch
import torch.nn as nn
...
def cw_l2_attack(model, images, labels, confidence=0.0,
                 binary_search_steps=8, max_iterations=200,
                 learning_rate=0.01, initial_const=1e-2,
                 device="cpu"):
    """
    Carlini & Wagner L2 attack: searches for the minimal
    L2 perturbation that flips the prediction, using a
    tanh change of variables, a logit margin loss, and
    ...
    one_hot[torch.arange(B, device=device), labels] = 1.0

    lower = torch.zeros(B, device=device)
    const = torch.full((B,), initial_const, device=device)
    upper = torch.full((B,), 1e10, device=device)

    best_l2 = torch.full((B,), float("inf"), device=device)
    best_adv = images.clone()
    best_ok = torch.zeros(B, dtype=torch.bool, device=device)
    ...
    for _ in range(binary_search_steps):
        ...
        prev = float("inf")
        step_ok = torch.zeros(B, dtype=torch.bool,
                              device=device)

        for it in range(max_iterations):
            adv = _from_tanh_space(w)
            l2_sq = (adv - images).flatten(1).pow(2).sum(1)
            logits = model(adv)
            ...
            if it % max(1, max_iterations // 10) == 0:
                if loss.item() > prev * 0.9999:
                    break
                prev = loss.item()

        # Binary search on c.
        with torch.no_grad():
            upper = torch.where(step_ok,
                torch.minimum(upper, const), upper)
            lower = torch.where(~step_ok,
                torch.maximum(lower, const), lower)
            has_up = upper < 1e9
            mid = (lower + upper) / 2
            const = torch.where(has_up, mid, const * 10)

    return best_adv.detach(), best_ok, best_l2
\end{lstlisting}
...
def evaluate_robustness(model, test_loader, attack_fn,
    ...
    """Evaluate model robustness across a list of epsilon values;
    returns a dict mapping each epsilon to accuracy (percent)."""
    model.eval()
    results = {}
